...
from utils.loss_normal import get_loss
...
def train(device,log_file):
    ...
    def normalize(data):
        ...
        normalizes = (data -input_mean)/input_std
        return normalizes
    ...
    for i in p_bar:
        ...
        # 计算所有损失
        interface_loss12 = loss.get_continuity_loss(net1, net2,normalize(dataset.sample_interface()[0]))
        interface_loss23 = loss.get_continuity_loss(net2, net3, normalize(dataset.sample_interface()[1]))
        
        boundary_lossin1 = loss.boundary_loss(net1, normalize(dataset.sample_boundary()[0]), T1)
        boundary_lossin2 = loss.boundary_loss(net2, normalize(dataset.sample_boundary()[0]), T1)
        boundary_lossin3 = loss.boundary_loss(net3, normalize(dataset.sample_boundary()[0]), T1)
        
        boundary_lossout1 = loss.boundary_loss(net1, normalize(dataset.sample_boundary()[1]), T2)
        boundary_lossout2 = loss.boundary_loss(net2, normalize(dataset.sample_boundary()[1]), T2)
        boundary_lossout3 = loss.boundary_loss(net3, normalize(dataset.sample_boundary()[1]), T2)
        
        PDE_loss1 = loss.pde_loss(net1, normalize(dataset.sample_pde()[0]), k1)
        PDE_loss2 = loss.pde_loss(net2, normalize(dataset.sample_pde()[1]), k2)
        PDE_loss3 = loss.pde_loss(net3, normalize(dataset.sample_pde()[2]), k3)

        gt_loss1 = loss.analytical_loss(net1,normalize(dataset.get_analytical_solutions()[0]),normalize(dataset.get_analytical_solutions()[1]))
        gt_loss2 = loss.analytical_loss(net2,normalize(dataset.get_analytical_solutions()[2]),normalize(dataset.get_analytical_solutions()[3]))
        gt_loss3 = loss.analytical_loss(net3,normalize(dataset.get_analytical_solutions()[4]),normalize(dataset.get_analytical_solutions()[5]))
        ...
        # 一次性计算所有梯度
        total_loss = total_loss1 + total_loss2 + total_loss3 + 3*total_loss4
        total_loss.backward()
        ...
        if i % 100 == 0:
            ...
            with open(log_file, 'a') as f:
                f.write(f'{i}\t{total_loss_val:.6f}\t{PDE_loss1.item():.6f}\t{PDE_loss2.item():.6f}\t'
                    ...
                    f'{boundary_loss_total:.6f}\t{analytical_loss_total:.6f}\n')
