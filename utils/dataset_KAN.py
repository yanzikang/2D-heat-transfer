...
class get_Dataset():
    ...
    def sample_boundary(self):
        with torch.no_grad():
            data = np.load("data/sampling_points_KAN.npy", allow_pickle=True).item()  # 将数组转换为字典
    ...
    def get_analytical_solutions(self):
        data = np.load("data/analytical_solutions_KAN.npy", allow_pickle=True).item()  # 将数组转换为字典
